...
import { 
  ...
  Filter, X, Save, Bookmark, LayoutGrid, List as ListIcon, Package as PackageIcon
} from 'lucide-react';

export const PackageList = () => {
  const { 
    activePackages, statuses, deletePackage, updatePackage, 
    statusColors, savedFilters, addSavedFilter, tableDensity, setTableDensity,
    isLoading
  } = usePackages();
  ...
  const [hasDocsFilter, setHasDocsFilter] = useState<'all' | 'yes' | 'no'>('all');
  ...
  const filteredPackages = useMemo(() => {
    return activePackages.filter(pkg => {
      ...
      const matchesDocs = hasDocsFilter === 'all' || 
        (hasDocsFilter === 'yes' ? pkg.documentsUploaded : !pkg.documentsUploaded);
        
      let matchesDate = true;
      if (dateRange.start && dateRange.end) {
        const pkgDate = pkg.dateSubmitted ? new Date(pkg.dateSubmitted) : null;
        if (pkgDate) {
          matchesDate = pkgDate >= new Date(dateRange.start) && pkgDate <= new Date(dateRange.end);
        } else {
  ...
  const getStatusColor = (status: string) => {
    return statusColors[status] || 'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300';
  };
    ...
    if (name) {
      addSavedFilter({
        ...
        filters: {
          ...
          hasDocs: hasDocsFilter !== 'all' ? (hasDocsFilter === 'yes') : undefined,
  ...
  const applySavedFilter = (filter: any) => {
    ...
    setHasDocsFilter(filter.filters.hasDocs !== undefined ? (filter.filters.hasDocs ? 'yes' : 'no') : 'all');
  ...
  const ProgressStepper = ({ status }: { status: string }) => {
    const steps = ['Pending', 'Customs Processed', 'Bond & Released'];
        ...
        {steps.map((step, index) => (
          <React.Fragment key={step}>
            <div className={`w-3 h-3 rounded-full ${
              index <= currentIndex ? 'bg-indigo-600 dark:bg-indigo-500' : 'bg-zinc-300 dark:bg-zinc-700'
            }`} title={step} />
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 border rounded-lg transition-colors text-sm font-medium ${
              showFilters || statusFilter !== 'all' || hasDocsFilter !== 'all' || dateRange.start
        ...
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 animate-in fade-in slide-in-from-top-2">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowFilters(false)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
                <X size={20} />
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">Documents Uploaded</label>
              <select
                value={hasDocsFilter}
            ...
            <div>
              <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">Date Submitted Range</label>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        {viewMode === 'table' ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10">
                <tr>
                  <th className={`px-6 font-medium text-zinc-500 dark:text-zinc-400 text-sm ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>Tracking / ID</th>
                  <th className={`px-6 font-medium text-zinc-500 dark:text-zinc-400 text-sm ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>Dates</th>
                  <th className={`px-6 font-medium text-zinc-500 dark:text-zinc-400 text-sm ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>Status</th>
                  <th className={`px-6 font-medium text-zinc-500 dark:text-zinc-400 text-sm ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>Docs</th>
                  <th className={`px-6 font-medium text-zinc-500 dark:text-zinc-400 text-sm text-right ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>Actions</th>
                </tr>
              </thead>
                ...
                {paginatedPackages.map((pkg) => (
                  <React.Fragment key={pkg.id}>
                    <tr className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group">
                      <td className={`px-6 ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>
                      ...
                      <td className={`px-6 ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>
                        <div className="text-sm flex flex-col gap-1">
                          <div className="flex items-center gap-1">
                            <span className="text-zinc-500 dark:text-zinc-400 w-8">Sub:</span>
                            {inlineEditId === pkg.id && inlineEditField === 'dateSubmitted' ? (
                              <input
                                autoFocus
                                type="date"
                              ...
                              <span 
                                ...
                                onClick={() => startInlineEdit(pkg.id, 'dateSubmitted', pkg.dateSubmitted)}
                              >
                          ...
                          <div className="flex items-center gap-1">
                            <span className="text-zinc-500 dark:text-zinc-400 w-8">Rel:</span>
                            {inlineEditId === pkg.id && inlineEditField === 'dateReleased' ? (
                              <input
                                autoFocus
                                type="date"
                                value={inlineEditValue as string}
                              ...
                              <span 
                                className="text-zinc-900 dark:text-zinc-200 cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400"
                                onClick={() => startInlineEdit(pkg.id, 'dateReleased', pkg.dateReleased)}
                              >
                      ...
                      <td className={`px-6 ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>
                        <div className="flex flex-col items-start gap-1">
                          ...
                          <select 
                            value={pkg.status}
                            onChange={(e) => handleStatusChange(pkg.id, e.target.value as Status)}
                      ...
                      <td className={`px-6 ${tableDensity === 'compact' ? 'py-2' : 'py-4'}`}>
                        <button 
                          onClick={() => updatePackage(pkg.id, { documentsUploaded: !pkg.documentsUploaded })}
                      ...
                      <td className={`px-6 ${tableDensity === 'compact' ? 'py-2' : 'py-4'} text-right`}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                  <p className="text-zinc-500 dark:text-zinc-400">Broker Form Status</p>
                                  <p className="font-medium text-zinc-900 dark:text-zinc-200">{pkg.brokerFormStatus || 'None'}</p>
                                </div>
                                <div>
                                  <p className="text-zinc-500 dark:text-zinc-400">Ready System Updated</p>
                                  <p className="font-medium text-zinc-900 dark:text-zinc-200">{pkg.readySystemStatusUpdated ? 'Yes' : 'No'}</p>
                                </div>
                                {pkg.expectedDutyAmount !== undefined && (
                                  <div>
                                    <p className="text-zinc-500 dark:text-zinc-400">Expected Duty</p>
                                    <p className="font-medium text-zinc-900 dark:text-zinc-200">${pkg.expectedDutyAmount.toFixed(2)}</p>
                                  </div>
                                )}
                                {pkg.clarificationDetails && (
                                  <div className="col-span-2">
                                    <p className="text-zinc-500 dark:text-zinc-400">Clarification Details</p>
                                    <p className="font-medium text-zinc-900 dark:text-zinc-200 mt-1 bg-white dark:bg-zinc-800 p-2 rounded border border-zinc-200 dark:border-zinc-700">{pkg.clarificationDetails}</p>
                                  </div>
                                )}
                                {pkg.cancellationReason && (
                                  <div className="col-span-2">
                                    <p className="text-zinc-500 dark:text-zinc-400">Cancellation Reason</p>
                                    <p className="font-medium text-rose-600 dark:text-rose-400 mt-1 bg-rose-50 dark:bg-rose-900/20 p-2 rounded border border-rose-100 dark:border-rose-800">{pkg.cancellationReason}</p>
                                  </div>
                                )}
                                {/* Render Custom Fields */}
                                {pkg.customFields && Object.entries(pkg.customFields).map(([key, value]) => (
                                  <div key={key}>
                                    <p className="text-zinc-500 dark:text-zinc-400">{key}</p>
                                    <p className="font-medium text-zinc-900 dark:text-zinc-200">{String(value)}</p>
                                  </div>
                                ))}
                              </div>
                            </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden">
                                <ul className="divide-y divide-zinc-100 dark:divide-zinc-700 max-h-40 overflow-y-auto">
                                  {pkg.history?.slice().reverse().map((entry, idx) => (
                                  ...
                                  {(!pkg.history || pkg.history.length === 0) && (
                                    <li className="px-3 py-2 text-sm text-zinc-500 dark:text-zinc-400 text-center">No history available</li>
                                  )}
                                </ul>
                              </div>
                            </div>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center text-zinc-500 dark:text-zinc-400">
                        <PackageIcon size={48} className="mb-4 text-zinc-300 dark:text-zinc-600" />
                        <p className="text-lg font-medium text-zinc-900 dark:text-zinc-200">No packages found</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-zinc-50 dark:bg-zinc-900/50">
            {paginatedPackages.map(pkg => (
              <div key={pkg.id} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 shadow-sm flex flex-col">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-semibold text-zinc-900 dark:text-zinc-100">{pkg.trackingNumber}</h3>
                    {pkg.rNumberIdNumber && <p className="text-sm text-zinc-500 dark:text-zinc-400">{pkg.rNumberIdNumber}</p>}
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(pkg)} className="p-1.5 text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-900/30">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDeleteClick(pkg.id)} className="p-1.5 text-zinc-400 hover:text-rose-600 dark:hover:text-rose-400 rounded-md hover:bg-rose-50 dark:hover:bg-rose-900/30">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-700">
                  <div>
                    <p className="text-zinc-500 dark:text-zinc-400 text-xs">Submitted</p>
                    <p className="font-medium text-zinc-900 dark:text-zinc-200">{pkg.dateSubmitted || '-'}</p>
                  </div>
                  <div>
                    <p className="text-zinc-500 dark:text-zinc-400 text-xs">Released</p>
                    <p className="font-medium text-zinc-900 dark:text-zinc-200">{pkg.dateReleased || '-'}</p>
                  </div>
            ...
            {paginatedPackages.length === 0 && (
              <div className="col-span-full py-12 text-center">
                <p className="text-zinc-500 dark:text-zinc-400">No packages found</p>
              </div>
            )}
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-zinc-900">
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Showing <span className="font-medium text-zinc-900 dark:text-zinc-100">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-medium text-zinc-900 dark:text-zinc-100">{Math.min(currentPage * itemsPerPage, filteredPackages.length)}</span> of <span className="font-medium text-zinc-900 dark:text-zinc-100">{filteredPackages.length}</span> results
            </p>
            <div className="flex gap-2">
              <button
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-2">Delete Package?</h3>
              <p className="text-zinc-600 dark:text-zinc-400 text-sm mb-6">
                This package will be moved to the Deleted tab. It will be permanently removed after 7 days.
              </p>
